...
# 导入原始数据集
dataset_dirs = ['/mnt/ve_share2/zy/language_table_dataset/0.1.0']
...
os.makedirs('/mnt/ve_share2/zy/language_table_np/data/train', exist_ok=True)
cnt = 0
for element in tqdm(ds.take(NUM_TRAIN)):
    create_episode(f'/mnt/ve_share2/zy/language_table_np/data/train/episode_{cnt}.npy',element)
    cnt = cnt + 1

print("Generating val examples...")
os.makedirs('/mnt/ve_share2/zy/language_table_np/data/val', exist_ok=True)
cnt = 0
for element in tqdm(ds.skip(NUM_TRAIN).take(NUM_VAL)):
    create_episode(f'/mnt/ve_share2/zy/language_table_np/data/val/episode_{cnt}.npy', element)
    cnt = cnt + 1

print("Generating test examples...")
os.makedirs('/mnt/ve_share2/zy/language_table_np/data/test', exist_ok=True)
cnt = 0
for element in tqdm(ds.skip(NUM_TRAIN + NUM_VAL).take(NUM_TEST)):
    create_episode(f'/mnt/ve_share2/zy/language_table_np/data/test/episode_{cnt}.npy', element)
    cnt = cnt + 1
